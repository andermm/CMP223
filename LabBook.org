...
#+end_example
**** PingPong - Latency
#+begin_src R :results output graphics :file SLIDES/Latency-paper.pdf :exports both :width 4 :height 4 :session *R* 
ggplot(df_intel_latency[df_intel_latency$interface %in% c("ib", "eth"), ],aes(x=bytes, y=average)) +
...
ggplot(df_intel_band[df_intel_band$interface %in% c("ib", "eth"), ],aes(x=bytes, y=average)) +
  ...
  theme (legend.position = c(0.1, 0.85),
         ...
         axis.text.x = element_text(size =16, color = "black"),
         axis.text.y = element_text(size =16, color = "black"))
ggsave("SLIDES/img/Bandwidth-paper.pdf")
#+end_src
