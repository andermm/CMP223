...
#+end_example
**** PingPong - Latency
#+begin_src R :results output graphics :file SLIDES/img/Latency-paper.pdf :exports both :width 4 :height 4 :session *R* 
library("scales")
ggplot(df_intel_latency[df_intel_latency$interface %in% c("ib", "eth"), ],aes(x=bytes, y=average)) +
...
**** PingPong - Bandwidth
#+begin_src R :results output graphics :file SLIDES/img/Bandwidth-paper.pdf :exports both :width 4 :height 4 :session *R* 
library("scales")
ggplot(df_intel_band[df_intel_band$interface %in% c("ib", "eth"), ],aes(x=bytes, y=average)) +
