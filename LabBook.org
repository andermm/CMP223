...
options(crayon.enabled=FALSE)
suppressMessages(library("tidyverse"));
df_apps <- read_csv("/home/anderson/Desktop/GIT/CMP223/LOGS/LOGS_CSV/apps_exec.22-11-2019.16h44m04s.csv", progress=FALSE)
...
df_apps %>%
  ...
  summarise(
    ...
    A8_h=0.975,
    A8_m=A8_h/60,
    A8_cost=A8_m*average_m,
    A10_h=0.78,
    A10_m=(A10_h/60),
    A10_cost=A10_m*average_m) %>%
  arrange(apps,interface) -> df_apps
df_apps
#+end_src
**** FT-EP-IS-SP
...
#+RESULTS:
[[file:R/PLOTS/Alya-IMB.charac.percentage.png]]
***** Alya-Percentage
...
#+RESULTS:
[[file:R/PLOTS/Alya.charac.percentage.png]]
...
#+RESULTS:
[[file:R/PLOTS/imb_CPU.charac.percentage.png]]
...
#+RESULTS:
[[file:R/PLOTS/imb_memory.charac.percentage.png]]
*** TOP500
...
ggplot(df, aes(y=N, x=TOP, fill=Interface)) +
  ...
  theme_bw() +
  scale_fill_manual(values=c("#7192ca", "#84b95b", "#808585", "#e0964b", "#d25e5f")) +
  theme(legend.position = c(0.15, 0.85),
        legend.background = element_rect(color = "black", size = 0.3, linetype = "solid"),
        plot.margin = unit(x = c(0, 0, 0, 0), units = "cm"),
        axis.text.x = element_text(size =16, color = "black"),
