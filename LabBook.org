...
**** IMB-CPU
#+begin_src R :results output graphics :file R/PLOTS/IMB-CPU.png :exports both :width 800 :height 400 :session *R* 
  library(tidyverse)
  df.imb.CPU <- read_csv("LOGS/TRACE/charac_imb_CPU.eth.csv", col_names = FALSE) %>%
  ...
  df.imb.mem <- read_csv("LOGS/TRACE/charac_imb_memory.eth.csv", col_names = FALSE) %>%
    mutate(Rank = gsub("MPI Rank", "", X2)) %>%
    select(-X1, -X2, -X3, -X7) %>%
    rename(Start = X4,
           End = X5,
  ...
  df.imb.CPU %>%
    ...
    arrange(Rank) -> df.imb.CPU.timings;
  df.imb.CPU.timings
 ...
 df.imb.mem %>%
  ...
  df.imb.CPU %>%
    ...
    mutate(Value = "Computing") -> df.imb.CPU.compute
  df.imb.CPU.compute
  ...
  df.imb.CPU %>%
    filter(Value != "MPI_Finalize") %>%
    group_by(Rank,Value) %>%
    summarize(Time = sum(Duration)) -> df.imb.CPU.communication;
  
  df.imb.CPU.communication$Value <- as.character(df.imb.CPU.communication$Value)
  df.imb.CPU.communication$Value [grepl('MPI_*', df.imb.CPU.communication$Value)] <- 'MPI'
  
  df.imb.CPU.communication %>%
    group_by(Rank, Value) %>%
    summarize(Time = sum(Time)) -> df.imb.CPU.communication;
  
  
  df.imb.CPU.compute %>%
    bind_rows(df.imb.CPU.communication) %>%
