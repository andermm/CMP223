...
options(crayon.enabled=FALSE)
suppressMessages(library(tidyverse))
df.ft <- read_csv("LOGS/TRACE/charac_ft.eth/charac_ft.eth.csv", col_names = FALSE) %>%
  mutate(Rank = gsub("MPI Rank", "", X2)) %>%
  select(-X1, -X2, -X3, -X7) %>%
  rename(Start = X4,
         End = X5,
...
df.ep <- read_csv("LOGS/TRACE/charac_ep.eth/charac_ep.eth.csv", col_names = FALSE) %>%
  mutate(Rank = gsub("MPI Rank", "", X2)) %>%
  select(-X1, -X2, -X3, -X7) %>%
  rename(Start = X4,
         End = X5,
...
df.is <- read_csv("LOGS/TRACE/charac_is.eth/charac_is.eth.csv", col_names = FALSE) %>%
  mutate(Rank = gsub("MPI Rank", "", X2)) %>%
  select(-X1, -X2, -X3, -X7) %>%
  rename(Start = X4,
         End = X5,
...
df.sp <- read_csv("LOGS/TRACE/charac_sp.eth/charac_sp.eth.csv", col_names = FALSE) %>%
  mutate(Rank = gsub("MPI Rank", "", X2)) %>%
  select(-X1, -X2, -X3, -X7) %>%
  rename(Start = X4,
         End = X5,
