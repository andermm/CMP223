...
- Factor 1 - Apps: The applications name totaling 13 (~charac_bt~,
  ...
  ~charac_ft~, ~charac_imb_memory~, ~charac_imb_CPU~, ~charac_ondes3d~, and
  ~charac_alya~).
- Factor 2 - Interface: The network interface name, totaling 3 (~eth~,
  ~ib~, ~ipoib~).
...
set.seed(0)
  cmp223_charac <- fac.design(factor.names = list(
    apps = c("charac_bt", "charac_ep", "charac_cg", "charac_mg", "charac_lu",
             ...
             "charac_imb_CPU", "charac_ondes3d", "charac_alya"),
    interface = c("eth", "ib", "ipoib")),
    replications=1,
    randomize=TRUE)
...
- [[SH/experiments_exec.sh][Experiments Execution]] - This is the most essential script for this
  evaluation. It comprises from the beggining, when the variables
  are defined, until the end, when the experiments execution ends and the
  results are classified. To describe it, it has been divided into a
  few steps, which are:

  - *Step 1*: Defining the Variables - Here all the variables with
    folder, software and bencharmark locations are defined.
  ...
  - *Step 3*: Create the Folders/Download and Compile the Programs -
    Here the priviously definied folders are created and the softwares
    and benchmarks are donwloaded and compiled with their respective
    compilers and with the Score-P + compilers.
  ...
  - *Step 5*: Read the Experimental Project and Start the Execution
    Loop - This final step reads the experimental project, start the MPI
...
execution]] script line 81), adjust the machine files in [[MACHINE_FILES][Machine Files]]
folder also with the name of the servers, adjust the number of
processes (in [[SH/experiments_exec.sh][experiments execution]] script lines 273, 276, 279 and,
282 and [[SH/experiments_charac.sh][experiments characterization]] script lines 97, 100 and, 103) to
be used during the execution, respecting the power of two or square
